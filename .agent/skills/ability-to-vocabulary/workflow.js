export const meta = {
  ...
  whenToUse: 'Procedure: .agent/skills/ability-to-vocabulary/SKILL.md. The owner has card designs whose abilities are written in plain language (a spreadsheet CSV, YAML, JSON or a Markdown table) and wants an importable card set plus the list of mechanics the vocabulary cannot express yet. args: {source, set, locale?, out?, batch?}',
  phases: [
    { title: 'Normalise', detail: 'read the designs, normalise them, split them into batches' },
...
const results = await pipeline(
  ...
  (translated, batch, i) => {
    ...
    return agent(
      `${GROUND}\n\nYou independently check a draft another agent wrote. Designs (plain ${LOCALE}): ${batch}. Draft: ${draft}. The translator's report: ${JSON.stringify(translated.cards)}\n\nRun \`cd server && uv run opengwt-data check-set ../${draft} --set ${SET} --locale ${LOCALE} --data ../data\`. For every card, compare the generated text with the design's own ability text: does the YAML say the same thing — trigger and timing, conditions, targets and sides, how many, how much, timers, charges, cooldown, where cards come from and go? Fix the draft in place where it misreads the design or fails the schema, and re-run until check-set is clean. Hold the draft to the translation conventions of .agent/skills/ability-to-vocabulary/SKILL.md §2. Do not add abilities that approximate an unmapped phrase; if the translator dropped something without reporting it, report it in extra_unmapped. verdict: faithful (nothing to change), fixed (you changed it; say what), lossy (part of the design is unmapped).`,
