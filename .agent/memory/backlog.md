...
- Phase B: two rows with capacity, base / boost / damage / armour, statuses with timers,
  ...
  dropped; data rewritten to v2; goldens regenerated. Remove the strict `xfail` markers phase A
  put on `test_packaged_schemas_match_docs` and `test_protocol_examples_load` — they fail loudly
  once the packaged schemas are v2.
