...
the server-hosted bot through `MatchClient`, taking every move from `legal_intents`, and needs
a server at `OPENGWT_TEST_SERVER`; `BoardViewTests` (PlayMode) needs no server — it feeds
